...
function sample(list, num) {
 ...
 for (let i = 0; i < num; i++){
  ...
  sampleList.push(list[randomNum])
 }
 return sampleList
}

function generateRandomCharacterList(list, num=1) {
 return sample(list, num).join(' ')
}
...
function startNextRound(button=null, nextRound) {
 ...
 let randomCharacterList = generateRandomCharacterList(numList, 3)
